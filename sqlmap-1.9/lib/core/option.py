...
def _cleanupOptions():
    ...
    # 确保kb.explicitSettings已经初始化
    if not hasattr(kb, "explicitSettings") or kb.explicitSettings is None:
...
def setVerbosity():
    ...
    # 从conf对象获取参数，而不是直接使用args
    conf.verbose = conf.get("verbose", 1)
    conf.debug = conf.get("debug", False)
    conf.aiCli = conf.get("aiCli", False)
    conf.smartPayload = conf.get("smartPayload", False)
    conf.aiAnalysis = conf.get("aiAnalysis", False)
    conf.explainVuln = conf.get("explainVuln", False)
    conf.suggestFix = conf.get("suggestFix", False)
...
def initOptions(inputOptions=AttribDict(), overrideOptions=False):
    ...
    conf.debug = False
    conf.aiCli = False
    conf.smartPayload = False
    conf.aiAnalysis = False
    conf.explainVuln = False
    conf.suggestFix = False
    
    # 如果inputOptions是Namespace对象（来自argparse），转换为字典
    if not isinstance(inputOptions, AttribDict):
        inputOptions = AttribDict(inputOptions.__dict__ if hasattr(inputOptions, '__dict__') else inputOptions)
    
    # 合并选项
    _mergeOptions(inputOptions, overrideOptions)
    
    # 确保所有必要的选项都存在
    for key in ("verbose", "debug", "aiCli", "smartPayload", "aiAnalysis", "explainVuln", "suggestFix"):
        if key not in conf:
            conf[key] = inputOptions.get(key, False)
