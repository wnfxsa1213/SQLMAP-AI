...
def cmdLineParser(argv=None):
    ...
    # AI模块参数
    aiGroup = parser.add_argument_group("AI", "These options can be used to customize AI module behavior")
    aiGroup.add_argument("--ai-cli", dest="aiCli", action="store_true",
                        help="启动AI交互式命令行界面")
    aiGroup.add_argument("--smart-payload", dest="smartPayload", action="store_true",
                        help="使用AI生成智能的SQL注入payload")
    aiGroup.add_argument("--ai-analysis", dest="aiAnalysis", action="store_true",
                        help="使用AI分析扫描结果")
    aiGroup.add_argument("--explain-vuln", dest="explainVuln", action="store_true",
                        help="提供检测到的漏洞的详细解释")
    aiGroup.add_argument("--suggest-fix", dest="suggestFix", action="store_true",
                        help="为检测到的漏洞提供修复建议")
