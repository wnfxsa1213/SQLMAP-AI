...
def shellExec(cmd):
    ...
    try:
        if subprocess.mswindows:  # Windows系统
        ...
        return stdout.decode(sys.getfilesystemencoding() or "utf-8")
...
def stdoutEncode(data):
    ...
    """
    try:
        # 对于Windows系统，尝试获取控制台代码页
        if sys.platform.startswith("win"):
            output = shellExec("chcp")
            if output and "active code page" in output.lower():
                encoding = "cp" + output.strip().split(": ")[1]
            else:
                encoding = sys.stdout.encoding or sys.getfilesystemencoding() or "utf-8"
        else:
            # 对于其他系统，使用标准输出编码
            encoding = sys.stdout.encoding or sys.getfilesystemencoding() or "utf-8"
        ...
        return data.encode(encoding, errors="replace").decode(encoding)
    except:
        return str(data)
